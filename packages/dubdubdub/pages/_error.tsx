...
export const RawError = ({ className, statusCode }: Props) => {
  if (statusCode === 404) {
    return (
      <GenericContainer className={className}>
        ...
        <Text>Typo? Bad link? Hacker? Either way, nothing belongs here that we know of.</Text>
  ...
  return (
    <GenericContainer className={className}>
      ...
      <Text>
        {/* prettier-ignore */}
        Something bad happened that we didn’t plan for. Try refreshing the page or go back. If this
        keeps happening, please send us a message at{" "}
        <Link href="mailto:finances@mike-engel.com">finances@mike-engel.com</Link>.
      </Text>
    </GenericContainer>
  );
};
...
RawError.getInitialProps = ({ res, err }: NextContext) => {
  const statusCode = res ? res.statusCode : err ? (err as Record<string, any>).statusCode : null;
